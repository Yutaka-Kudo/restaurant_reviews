...
def scrape_google_test():
    ...
    # page_range = range(20,30)
    page_range = range(1, 30)

    # page_range = range(6,30)
        ...
        for page_num in page_range:  # ページーーーーーーーーーーーーー
            ...
            for store_num in range(1, 21):  # 店ーーーーーーーーーーーーーーーー
                ...
                if first_page_flg is True and store_num == ad_count + 4:
                    ...
                    ad = driver.find_element_by_xpath("//span[a[contains(text(),'所在地')]]/following-sibling::span").text
                    zipcode = re.search("〒?\d{3}-\d{4}",ad).group()
                    ad.replace(zipcode,"").strip()
                    ad = " ".join(driver.find_element_by_xpath("//span[a[contains(text(),'所在地')]]/following-sibling::span").text.split(' ')[1:])
                    ...
                    # try:
                    #     address = driver.find_element_by_
                    #     # print(address)
                    # except Exception:
